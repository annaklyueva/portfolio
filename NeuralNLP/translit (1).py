...
from score_translit import load_dataset, TRANSLIT_PATH
...
class PositionalEncoding(nn.Module):
    def __init__(self, hidden_size, max_len=512):
        ...
        # took the implementation from https://medium.com/swlh/elegant-intuitions-behind-positional-encodings-dc48b4a4a5d1
        ...
        pe = torch.reshape(pe, (1, max_len, hidden_size))
...
class MultiHeadAttention(nn.Module):
    ...
    def attention(self, query, key, value, mask):
        ...
        attn_weights = attn_weights/np.sqrt(self.head_hidden_size)
        if mask is not None:
...
class LrScheduler:
    def __init__(self, n_steps, **kwargs):
        ...
        if self.type == 'warmup,decay_linear':
            ...
            self.warmup = np.int(n_steps * self.warmup_steps_part)
    ...
    def learning_rate(self, step=None):
        ...
        if self.type == 'warmup,decay_linear':
            ## TODO: provide your implementation here
            if step > self.warmup:
                self._lr = self.lr_peak - (self.lr_peak/ (self.n_steps - self.warmup)* (step - self.warmup))
            else:
                self._lr = step * self.lr_peak / self.warmup
        return self._lr
...
def train(source_strings, target_strings):
    ...
    # prepare dev_small data
    dev_data = load_dataset(TRANSLIT_PATH, parts=('dev_small', ))
    _, strings_dev, translit_dev = dev_data['dev_small']

    dev_source_strings, dev_target_strings = zip(*sorted(zip(strings_dev, translit_dev),
                                                 key=lambda e: len(e[0])))

    dev_small_dataloader = create_dataloader(dev_source_strings, dev_target_strings, text_encoder,
                                             train_config['batch_size'], 
                                             shuffle_batches_each_epoch=False)



    name_dev = 'dev_b/acc_dev_'+str(len(os.listdir('dev_b/')))+'.txt'
    # training cycle
    for epoch in range(1,train_config['n_epochs']+1):
        ...
        #evaluation on dev_small
        max_decoding_len = model.config['max_tgt_seq_length']
        model.eval()
        predictions = generate_predictions(dev_small_dataloader, max_decoding_len, text_encoder, model, device)

        dev_metrics = compute_metrics(predictions, dev_target_strings, ['acc@1'])
        with open(name_dev, "a") as myfile:
            myfile.write(str(dev_metrics['acc@1'])+'\n')
        # print(dev_metrics)
